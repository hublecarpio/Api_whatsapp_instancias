...
async function sendWhatsAppResponseDirect(
  ...
  if (!instance) {
    throw new Error(`Instance ${instanceId} not found in business instances`);
  }
  ...
  if ((instance.provider === 'META_CLOUD' || instance.provider === 'META_COEXIST') && (metaCredential || coexistCredential)) {
    ...
    if (metaCredential) {
      ...
      throw new Error(`No valid credential found for instance ${instanceId}`);
    }
    ...
    throw new Error(`No valid provider/credentials for instance ${instanceId}`);
  }

  if (sendFailures > 0 && sendFailures === events.length) {
    throw new Error(`All ${sendFailures} message events failed to send. Last error: ${lastSendError}`);
  }
}
