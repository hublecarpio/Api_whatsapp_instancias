...
export default function AsesorPage() {
  ...
  const loadConversations = async () => {
    ...
    try {
      ...
      setHasMoreConversations(res.data?.hasMore || false);
    } catch (error) {
  ...
  const loadMoreConversations = async () => {
    if (!selectedBusiness || loadingMoreConversations || !hasMoreConversations) return;
    setLoadingMoreConversations(true);
    try {
      const res = await messageApi.conversations(selectedBusiness.id, undefined, {
      ...
      <div className="flex-1 flex overflow-hidden relative">
        ...
        `} style={{ top: 0 }}>
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            {hasMoreConversations && !searchQuery && !selectedTag && (
              <button
                onClick={loadMoreConversations}
                disabled={loadingMoreConversations}
                className="w-full py-3 text-sm text-neon-blue hover:bg-dark-hover transition-colors disabled:opacity-50 border-t border-dark-border"
              >
