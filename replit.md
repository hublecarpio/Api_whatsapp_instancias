...
*   **Plan Type Differentiation**: The `/me` endpoint returns a `planType` field ('pro' | 'basic' | 'trial' | 'none') with priority: Enterprise (via referral code/proBonusExpiresAt) > Basic (Stripe subscription) > Trial. The sidebar displays differentiated badges: "Pro" (green) for enterprise users, "Basic" (blue) for Stripe subscribers. Enterprise access never auto-cancels Stripe subscriptions.
