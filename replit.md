...
*   **Enhanced Mass Broadcast System**: Supports direct file upload to MinIO for images/videos/audio/documents, text+media together with captions, CSV contact import (phone,var1,var2 format), variable interpolation ({{1}}, {{2}}) for personalized messages, CRM metadata integration with named variables ({{nombre}}, {{email}}, {{custom_field}}) for CRM contacts, available variables endpoint for UI preview, frontend and backend deduplication, and Meta template component preservation.
